...
import { telugu100 } from "../../../constants/numbersData";
import { BigDisplay } from "../../shared/learnStyles";
import { SurpriseCard } from "../../../components/SurpriseCard";

const FACTS = [
  { emoji: "🔢", fact: "Counting is fun in every language!" },
  { emoji: "✨", fact: "ప్రతి సంఖ్యకు ఒక ప్రత్యేక పేరు ఉంటుంది. (Each number has a unique name)" },
  { emoji: "🎈", fact: "We can count all the way to 100!" },
];

const NumbersTeluguPage = () => {
  const [index, setIndex] = useState(0); // 0 to 99
  const current = telugu100[index];

  const handleNext = () => setIndex((p) => (p + 1) % 100);
  const handlePrev = () => setIndex((p) => (p - 1 + 100) % 100);

  return (
    <PageContainer data-testid="view-numbers-telugu">
      <GameLayout>
        <ChallengeHeader
          icon={Hash}
          title="అంకెలు (1-100)"
          subtitle="Learn Telugu numbers and spellings!"
          streak={0}
        />

        <SurpriseCard
          title="చిట్కా! 💡"
          subtitle="అంకె ఎలా పలకాలో వినడానికి స్పీకర్‌ని నొక్కండి! (Tap speaker to hear!)"
        />
        ...
        <GameActivityArea data-testid="activity-area">
          ...
          <NavControlBar>
            ...
            <SpeakIcon text={current.spell} lang="te-IN" />
            ...
            >
              {telugu100.map((num, i) => (
                <Tag key={num.id} $isActive={index === i} onClick={() => setIndex(i)}>
                  <span style={{ minWidth: "auto", padding: "8px 4px", fontSize: "0.75rem" }}>
                    {num.id}
                  </span>
                </Tag>
              ))}
...
export default NumbersTeluguPage;
